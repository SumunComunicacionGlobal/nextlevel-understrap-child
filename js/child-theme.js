...
(function (global, factory) {
  ...
  jQuery(function ($) {
    ...
    $(".single-formacion .entry-content .wp-block-list").each(function () {
      var box = $(this);
      var initialHeight = box.innerHeight(); // reduce the text if it's longer than 200px
    ...
    function SliderReadMore() {
      $(".slide-read-more-button").on('click', function () {
        var box = $(this).parent().prev('.slide-read-more'); // get current height

        var currentHeight = box.innerHeight(); // get height with auto applied

        var autoHeight = box.css('height', 'auto').innerHeight(); // reset height and revert to original if current and auto are equal
